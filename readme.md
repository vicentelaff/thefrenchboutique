**NB**: Pour lancer le serveur symfony, utiliser la commande `symfony serve`.
...
- On modifie la vue correspondante dans le dossier templates comme on le souhaite.
  **P.S.**: On modifie le dossier `twig.yaml` afin d'intégrer le thème de Bootstrap 4 à nos formulaires.
...
- Dans `SearchType.php`, on instancie donc la classe Search afin d'afficher à l'utilisateur (sur la vue index.html.twig dans template/products) un menu à gauche de l'écran lui permettant de réaliser une recherche ou de filtrer les produits selon leurs catégories.
  **P.S.:** On a besoin de créer une nouvelle fonction dans ProductsRepository qui va nous permettre de gérer notre recherche (`findWithSearch()`).
  > Commit et push sur notre repository GIT.
...
- Modifier statut de la commande:
    1. Dans Order.php on créé une nouvelle propriété `state` (à la place de `isPaid`), puis on rajoute le code suivant dans OrderCrudController:
...
```
    ...
    2.  On créé aussi deux nouvelles actions personnalisées (admin dashboard), voir OrderCrudController.

- Produits à la une (depuis le back office):
    1.  On rajoute une propriété `isBest` à l'entité Products et on la rajoute au ProductsCrudController.
    2.  Dans HomeController on rajoute le code suivant:
...
- Gestion du slider de la page d'accueil (depuis le back office):
    1. On créé une nouvelle entité `Header` ainsi qu'un `HeaderCrudController` à l'aide du terminal.
